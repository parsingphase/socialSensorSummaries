...
function analyzeWeatherData(
	weatherData: AmbientWeatherInterval[],
): IntervalSummary {
	...
	const duration = DateTime.fromMillis(maxTime)
		.diff(DateTime.fromMillis(minTime))
		.as("hours");
...
async function buildWeatherSummaryForDay(
	...
	const weatherData = await fetchDeviceWeatherRecords(
