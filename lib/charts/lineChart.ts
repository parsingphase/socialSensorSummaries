...
import { type Canvas, type CanvasRenderingContext2D, DOMMatrix } from "canvas";
...
function dateToLeapYearDayOfYear(date: DateTime): number {
	...
	return (
		Interval.fromDateTimes(startOfLeapYear, leapYearDate).count("days") + 1
	);
}
 ...
 * @param window
 */
function smoothCount<T extends { count: number | null }>(
	data: T[],
	window: number,
): T[] {
...
class LineChart extends ChartImageBuilder {
	...
	constructor(
		...
		this.smoothData = this.mapDataToYears(
			smoothCount<DatedCount>(speciesData, 7),
		);
	}
	...
	public drawGraph(): Canvas {
		...
		for (let i = 0; i < years.length; i++) {
			...
			const yearLegendLeft =
				this.graphOffset.x +
				(i * (this.graphWidth - this.graphOffset.x * 2)) / years.length;
			...
			ctx.lineTo(
				yearLegendLeft + this.legendBarLength,
				this.canvasHeight - ybase,
			);
			ctx.stroke();
	...
	protected plotPoints(
		...
		if (startDatum && startDatum.count !== null) {
			const startPoint = this.graphPointToCanvasXY(
				startDatum.xIndex,
				startDatum.count,
			);
...
export { LineChart, type DatedCount };
